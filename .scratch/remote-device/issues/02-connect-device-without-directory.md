...
- [x] 连接操作不再要求选择目录作为前提 —— **已打通**（设置页设备区块的「连接」按钮 → Root → useRemoteWorkspaceHistory.connectRemoteDevice，不 bind 目录/不建 tab/不写远程历史）
...
脚本：`packages/desktop/test/acceptance-device-connect.ts`

```
✅ 设备级连接成功（无目录，1116ms）
✅ 设备访问：已登记 10 个项目 / 枚举 33 条会话 / 投影清单 10 个
✅ 已断开
✅ 重连成功，单项目查询返回 8 条
```

关键实现（commits b7d752f / 4129b7c / b9d9e93 / dabd1d1）：
- 设备条目类型 `remoteDevice`（无 workspacePath，只含连接入口 + 显示偏好）
- 设备访问层：能力探测 + 退化（兼容旧对端）+ 项目清单并集
- 设备管理 UI：添加/编辑/移除/连接/断开/项目勾选
- 连接链路：`connectRemoteDevice`（复用既有连接通路，不 bind 目录）

残留：UI 视觉需人工确认（电脑操作为闭源占位，无自动化能力）。
